...
public class OracleConnectionPool {
  ...
  public OracleConnectionPool(final int option, final File XMLFilePath) {
...
//        e1.printStackTrace();
          Logger.write("OracleConnectionPool.OracleConnectionPool (Exception e1) :: " + e1.getMessage());
      }
//      e.printStackTrace();
      Logger.write("OracleConnectionPool.OracleConnectionPool (Throwable) :: " + e.getMessage());
    }finally{
      try {
    	  if(inputStream != null)
    		  inputStream.close();
...
//        e.printStackTrace();
        Logger.write("OracleConnectionPool.OracleConnectionPool (IOException) :: " + e.getMessage());
      }
    }
  }
