...
public class OracleConnectionPoolConsumosReplica {
  
  private BasicDataSource  basicDataSource = null;
  private DataSource       dataSource;

  public OracleConnectionPoolConsumosReplica(final int option, final File XMLFilePath) {
    ...
    try {
      ...
      basicDataSource.setDriverClassName(propertiesConnection.getProperty("driverConsumosReplica"));
	  basicDataSource.setUsername(propertiesConnection.getProperty("userConsumosReplica"));
	  basicDataSource.setPassword(propertiesConnection.getProperty("passwordConsumosReplica"));
	  basicDataSource.setUrl(propertiesConnection.getProperty("urlConsumosReplica"));  
...
//          e1.printStackTrace();
          Logger.write("OracleConnectionPoolConsumosReplica.OracleConnectionPoolConsumosReplica (IOException e1) :: " + e1.getMessage());
      }
//      e.printStackTrace();
      Logger.write("OracleConnectionPoolConsumosReplica.OracleConnectionPoolConsumosReplica (Throwable) :: " + e.getMessage());
    }finally{
      try {
    	  if(inputStream != null)
    		  inputStream.close();
...
//        e.printStackTrace();
          Logger.write("OracleConnectionPoolConsumosReplica.OracleConnectionPoolConsumosReplica (IOException) :: " + e.getMessage());
      }
    }
  }
