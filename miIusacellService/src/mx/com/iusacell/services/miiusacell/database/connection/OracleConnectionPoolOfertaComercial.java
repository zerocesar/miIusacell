...
public class OracleConnectionPoolOfertaComercial {
  
  private BasicDataSource  basicDataSource = null;
  private DataSource       dataSource;

  public OracleConnectionPoolOfertaComercial(final int option, final File XMLFilePath) {
    ...
    try {
      ...
      basicDataSource.setDriverClassName(propertiesConnection.getProperty("driverOfertaComercial"));
      basicDataSource.setUsername(propertiesConnection.getProperty("userOfertaComercial"));
      basicDataSource.setPassword(propertiesConnection.getProperty("passwordOfertaComercial"));
      basicDataSource.setUrl(propertiesConnection.getProperty("urlOfertaComercial"));
...
//        e1.printStackTrace();
          Logger.write("OracleConnectionPoolOfertaComercial.OracleConnectionPoolOfertaComercial (IOException e1) :: " + e1.getMessage());
      }
//      e.printStackTrace();
      Logger.write("OracleConnectionPoolOfertaComercial.OracleConnectionPoolOfertaComercial (Throwable) :: " + e.getMessage());
    }finally{
      try {
    	  if(inputStream != null)
    		  inputStream.close();
...
//        e.printStackTrace();
          Logger.write("OracleConnectionPoolOfertaComercial.OracleConnectionPoolOfertaComercial (IOException) :: " + e.getMessage());
      }
    }
  }
