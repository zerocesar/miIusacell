	...
	public static String respCodePrepagoLegacy(final String code){
		...
		if(banderaEjecutaCondicion == 1){
			if(code.equals("0")){
				...
				response = "Número de celular inválido";
			}
			else if(code.equals("4")){
				response = "Servicio no disponible";
			}
			else if(code.equals("5")){
				response = "Clave inválida";
			}
			else if(code.equals("6")){
				response = "Canal inválido";
			}
			else if(code.equals("10")){
				response = "Longitud del servicio inválido";
			}
			else if(code.equals("11")){
				response = "Servicio complementario inválido ";
			}
	...
	public static void validaTiempoDeVida(final String time, final int elapseTime) throws ServiceException{
		...
		try{
			...
			if(diffMinutes <= elapseTime){
				Logger.write("Debe esperar "+elapseTime+" minutos para poder ejecutar la acción");
				throw new ServiceException("Debe esperar "+elapseTime+" minutos para poder ejecutar la acción");
			}
...
//		if(mensaje.contains("ha excedido el tiempo máximo") 
//			|| mensaje.contains("no está conectado") 
//			|| mensaje.contains("no hay más datos para leer del socket")
//			|| mensaje.contains("está en un estado inconsistente")
//			|| mensaje.contains("tubería rota")
//			|| mensaje.contains("connection reset")
//			|| mensaje.contains("expiró el tiempo de conexión")
//			)
//		{
//			valida = true;
//		}
...
//	}
	...
	public static String retornacompania(final int idServicio){
		OracleProcedures oracle = new OracleProcedures();
		String compania="";
		Logger.write(" >>> C o n s u l t a  C o m p a ñ i a ");
		try {
			compania=oracle.retornaDescripcionCompania(idServicio+"", new MensajeLogBean());
			if (compania.equals("0")){
				Logger.write("[ctrl]  :: No se encontró descripcion de la UnidadNegocio :"+idServicio);
				compania="";
			}else {
